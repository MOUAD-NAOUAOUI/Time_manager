...
| `JWT_SECRET` | *(required, no default, min 32 chars)* | Secret key for signing JWTs |
...
| `AI_SERVICE_INTERNAL_TOKEN` | *(required, no default)* | Shared secret between Java & Python |
| `APP_ENCRYPTION_KEY` | *(required, no default, 32-byte Base64)* | AES-256 key for database column encryption |
